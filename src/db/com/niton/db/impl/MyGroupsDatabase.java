...
/**
 * This class is coordinating the acivities if a group gets created 
 * or if a list of group members should be returned 
...
public class MyGroupsDatabase {
	...
	/**
	 * @return a list of all groups who the user who is accessing the database created
	 */
	public ArrayList<ReducedGroup> list() {
		...
		for(GroupRecord f : rg){
			...
			pushing.description(f.getDescription()).name(f.getName()).creationDate(f.getCreationDate().toLocalDate()).members(getGroupMembers(f.getUid()).size()).tasks(getGroupTasks(f.getUid()).size());
			list.add(pushing);
		}
		return list;
	}
